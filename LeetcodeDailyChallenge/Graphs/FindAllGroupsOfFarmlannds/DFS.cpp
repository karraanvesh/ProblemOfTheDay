...
// Problem Link : https://leetcode.com/problems/find-all-groups-of-farmland/?envType=daily-question&envId=2024-04-20
    ...
    map<int , pair<int,int>> &mp , int n , int m) {
        ...
        for(int k = 0 ; k < 4 ; k++) {
            ...
            if(isSafe(newRow , newCol , n , m) and land[newRow][newCol] and !visited[newRow][newCol]) {
                dfs(newRow , newCol , land , visited , num , mp , n , m);
            }
        }
        ...
        for(int i = 0 ; i < n ; i++) {
            ...
            for(int j = 0 ; j < m ; j++) {
                ...
                if(land[i][j] and !visited[i][j]) {
                    ...
                    dfs(i , j , land , visited , num , mp , n , m);
